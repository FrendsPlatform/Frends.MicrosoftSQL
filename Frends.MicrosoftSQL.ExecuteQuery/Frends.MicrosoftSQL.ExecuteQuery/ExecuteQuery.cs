    ...
    private static async Task<Result> ExecuteHandler(Input input, Options options, SqlCommand command, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            else
            {
                ...
                {
                    var rbEMsg = $"ExecuteHandler exception: An exception occurred on transaction rollback. Rollback exception: {rollbackEx}. ||  Original exception leading to rollback: {ex}";
                    if (options.ThrowErrorOnFailure)
